...
(function(){
 ...
 const flyer=e=>e.flyer|| (e.id==='evt_garage_grinder_2'?'flyer-garage-grinder.jpg':e.id==='evt_lucky_u_motorcycle_pull'?'flyer-lucky-u.jpg':'flyer-state-championship.jpg');
 ...
 async function loadEvents(){try{const d=await api14('/api/events');events=(d.events||[]).filter(e=>e.date>='2026-09-09').sort((a,b)=>a.date.localeCompare(b.date)||String(a.startTime||'').localeCompare(String(b.startTime||''))||String(a.createdAt||'').localeCompare(String(b.createdAt||'')));window.__FAA_REG_EVENTS=events;renderHome();renderCatalog();renderSchedule();renderEventSelect();if(selectedId)renderEventDetail(eventById(selectedId));}catch(e){const n=document.getElementById('nextEventHero');if(n)n.innerHTML='<div class="body"><div class="notice">Unable to load the FAA schedule. Please refresh.</div></div>';}}
...
(()=>{
  ...
  function detail(e){return `<div class="launch-detail"><img src="${flyer(e)}" class="launch-flyer" alt="${esc(e.name)} flyer"><div class="card body"><div class="red">FAA EVENT</div><h1>${esc(e.name)}</h1><div class="small"><b>${fmtDate(e.date)}</b><br>${esc(e.location||'')}<br>${esc(e.address||'')}</div><div class="launch-stats"><div><b>$${e.entryFee??e.fee??0}</b><span>${esc(e.entryUnit||e.unit||'entry')}</span></div><div><b>${e.startTime||e.start||'See flyer'}</b><span>${e.startTime||e.start?'Start time':'Event time'}</span></div><div><b>FAA</b><span>Official event</span></div></div><h3>WEIGHT CLASSES</h3><div class="launch-pills big">${WEIGHTS.map(x=>`<span>${x}</span>`).join('')}</div><h3>CATEGORIES</h3><div class="launch-pills big">${CATS.map(x=>`<span>${x}</span>`).join('')}</div><h3>EVENT NOTES</h3><div class="small">${esc(e.desc)}</div><h3>PRIZES</h3><ul class="launch-prizes">${e.prizes.map(x=>`<li>${esc(x)}</li>`).join('')}</ul><button class="cta" data-go="register" data-prefill-event="${e.id}">REGISTER FOR THIS EVENT →</button><button class="cta outline" data-go="events">← BACK TO EVENTS</button></div></div>`}
  function schedule(){const list=window.__FAA_REG_EVENTS||EVENTS;return `<div class="launch-schedule">${list.map((e,i)=>`<button class="schedule-row" data-launch-event="${e.id}"><div class="schedule-num">${String(i+1).padStart(2,'0')}</div><div class="grow"><b>${fmtDate(e.date)}</b><strong>${esc(e.name)}</strong><span>${esc(e.location)}</span></div><div class="schedule-arrow">→</div></button>`).join('')}</div>`}
  function photos(){return '<div class="notice">Official photos uploaded by Florida Armwrestling will appear here.</div>'}
  ...
  async function refreshPublic(){
    ...
    const mapped=list.map(x=>Object.assign({},x,{flyer:x.flyer||eventById(x.id)?.flyer||'',entryFee:x.entryFee??x.fee??eventById(x.id)?.entryFee??eventById(x.id)?.fee??0,entryUnit:x.entryUnit||x.unit||eventById(x.id)?.entryUnit||eventById(x.id)?.unit||'entry',startTime:x.startTime||x.start||eventById(x.id)?.startTime||eventById(x.id)?.start||'',doorsTime:x.doorsTime||eventById(x.id)?.doorsTime||'',description:x.description||x.desc||eventById(x.id)?.description||'',prizes:Array.isArray(x.prizes)?x.prizes:(eventById(x.id)?.prizes||[]),weightClasses:Array.isArray(x.weightClasses)?x.weightClasses:(eventById(x.id)?.weightClasses||WEIGHTS),categories:Array.isArray(x.categories)?x.categories:(eventById(x.id)?.categories||CATS),status:x.status||eventById(x.id)?.status||'open'}));
  ...
  window.go=function(id){
    ...
    if(id==='events'&&$('eventCatalog')){$('eventCatalog').innerHTML=(window.__FAA_REG_EVENTS||EVENTS).map(card).join('')||'<div class="notice">No events scheduled.</div>';document.querySelectorAll('[data-launch-event]').forEach(b=>b.onclick=()=>openEvent(b.dataset.launchEvent));}
